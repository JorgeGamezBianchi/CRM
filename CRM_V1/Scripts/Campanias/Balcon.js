...
function SetBalconData(Id, FolioBalcon, ProductoBalcon, Ult4DgtBalcon, TasaAnualBalcon, CatBalcon,
    FechaCatBalcon, MontoMaximoBalcon, TipoVentaBalcon, EstatusVenta, FechaVisitaSucursal, EstadoSucursal, MunicipioSucursal,
    DivisionB, SucursalBN, NumeroSucursalBN, Calificacion) {

    LoadCombosBALCON(24, 0, "comboBalconCalif", Calificacion);
    ...
    $("#FechaCatBalcon").val(FechaCatBalcon);
    $("#MontoMaximoBalcon").val(MontoMaximoBalcon);
    $("#selectVentaB").val(TipoVentaBalcon);
    $("#SucursalBN").val(SucursalBN);
    $("#NumeroSucursalBN").val(NumeroSucursalBN);
    $("#RegionalBN").val(RegionalBN);
    $("#FechaVicitaSucursalBN").val(FechaVisitaSucursal);
    $("#EstadoSucursalBN").val(EstadoSucursal);
    $("#MunicipioSucursalBN").val(MunicipioSucursal);
    $("#SucursalB").val(SucursalBN);
    ...
    document.getElementById("check_bal").onclick = function () {
        check_validacion();
    }
    document.getElementById("SaveCalif").onclick = function () {
        saveCalif();
    }
    ...
    switch (EstatusVenta) {
        ...
        case 0://ya se vendio, no se ha validado
            $("#FechaVicitaSucursalBN").val(formatDate(FechaVisitaSucursal));
            $("#FechaVicitaSucursalBN").datepicker(cal);
            ...
            var TipoLlamadaBalcon = $("#selectVentaB");
            ...
            var FechaVisitaSucursal = $("#FechaVicitaSucursalBN");
            ...
            datos = {
                ...
                FechaVisitaSucursal: FechaVisitaSucursal.val(),
...
function ValidateBalconData() {
    ...
    if (FechaVisitaSucursal.val() === "") {
        alert("Ingrese fecha de visita");
        FechaVisitaSucursal.focus();
        return;
    }
...
function saveBalconData(json) {
    console.log("Entra saveBalcon");
    showLoader();
    var xml = new XMLHttpRequest();
    var url = hostInit + "/Client/SaveBalconData";
...
function sucu() {
    var cod = document.getElementById("selectVentaB").value;
    if (cod == "2") {
        document.getElementById("visitaSucursalB").hidden = false;
        document.getElementById("divEstadoB").hidden = false;
        document.getElementById("divMunicipioB").hidden = false;
        document.getElementById("divSucursal2B").hidden = false;

    } else {
        document.getElementById("visitaSucursalB").hidden = true;
        document.getElementById("divEstadoB").hidden = true;
        document.getElementById("divMunicipioB").hidden = true;
        document.getElementById("divSucursal2B").hidden = true;
    }
}
...
function LoadCombosBALCON(ID, CATALOGO, idCombo, dato) {
    var json;
    if (ID == -1 && CATALOGO == -1) {
        json = {}
    } else {
...
function check_validacion() {
    if ((document.getElementById('check_bal').checked) == true)
    {
        document.getElementById("SaveBalcon").hidden = false;
        document.getElementById("panelBALCON").hidden = false;
        document.getElementById("SaveCalif").hidden = true;
        document.getElementById("panelCalificacion").hidden = true;
    }
    else
    {
        document.getElementById("SaveBalcon").hidden = true;
        document.getElementById("panelBALCON").hidden = true;
        document.getElementById("SaveCalif").hidden = false;
        document.getElementById("panelCalificacion").hidden = false;
    }
}
//GUARDAR DATOS Y LLENAR JSON
function saveCalif() {
    var BalconCalif;
    BalconCalif = $("#comboBalconCalif").val;
    var FolioBalcon = $("#FolioBalcon");
    var Calificacion = document.getElementById("comboBalconCalif").value;
    
    if (Calificacion == 0) {
        alert("Seleccione UnaCalificacion");
        $("#comboBalconCalif").focus();
        return;
    }
    var json = {
        Folio: FolioBalcon.val(),
        NumeroCliente: document.getElementById("Cliente").value,
        Calificacion: document.getElementById("comboBalconCalif").value,
    };
    console.log(json);
    saveCalifData(json);
    datos = json;
...
function saveCalifData(json) {
    console.log("Entra saveBalcon calif");
    showLoader();
    ...
    xml.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            var result = JSON.parse(this.responseText);
            console.log(result);
            Calificado("EstadoBalcon", "HeadingBalcon", "SaveCalif", "SaveBalcon");
            hideLoader();
        }
    };
    xml.open('POST', url, true);
...
//    var xml = new XMLHttpRequest();
//    var url = hostInit + "/Client/SaveBalconCalif";
//    xml.onreadystatechange = function () {
//        if (this.readyState == 4 && this.status == 200) {
//            var result = JSON.parse(this.responseText);
//            console.log(result);
//            Calificado("EstadoBalcon", "HeadingBalcon", "ValidarBalcon", "SaveBalconCalif");
//            hideLoader();
//            //ValidarGrupo();
//        }
//    };
